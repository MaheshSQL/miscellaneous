# About: Create a handoff workflow (triage agent) of persistent Foundry agents with tool use capability
# Dependancy: A03_Create_Multiple_Foundry_Agent_Persistent.py (to create the agents first)
# Ref: https://learn.microsoft.com/en-us/agent-framework/user-guide/workflows/orchestrations/handoff
...
triage_agent_id = 'asst_YKbXfDuwSApRIINJqrZPJtzj'
...
async def main():
    async with (
        ...
        triage_foundry_agent = await project_client.agents.get_agent(agent_id=triage_agent_id)
        ...
        try:
            ...
            triage_chat_agent = ChatAgent(
                chat_client=AzureAIAgentClient(
                    project_client=project_client,
                    agent_id=triage_foundry_agent.id
                ),
                instructions=triage_foundry_agent.instructions, # From existing agent or can be overridden here
                # tools=None,
                name=triage_foundry_agent.name,
                description=triage_foundry_agent.description
            )


            print("Chat agents created successfully.")

            print('!! PENDING: Handoff workflow (Python) not yet supported in agent-framework, only supported in .NET currently !!')            
            # PENDING: Handoff workflow (Python) not yet supported in agent-framework, only supported in .NET currently
