...
endpoint = os.environ["PROJECT_ENDPOINT"] # Has to be Foundry project (not Hub-based project)
model_deployment_name = os.environ["MODEL_DEPLOYMENT_NAME"]

openapi_weather_spec = ".\\openapi_spec\\weather.json"
openapi_countries_spec = ".\\openapi_spec\\countries.json"
...
with AIProjectClient(
    ...
    with open(os.path.join(os.path.dirname(__file__), openapi_weather_spec), "r") as f:
    ...
    with open(os.path.join(os.path.dirname(__file__), openapi_countries_spec), "r") as f:
    ...
    # For authentication options refer to:
    # https://learn.microsoft.com/en-us/python/api/azure-ai-agents/azure.ai.agents.models.openapitool?view=azure-python-preview
    ...
    # # To test connection to Foundry project, uncomment the following lines:
    ...
    agent_id = 'asst_Rpw86PbD9RWwdKoyVfAcJw9D'
    ...
    if agent:
        ...
        name="openapi_spec_agent", # Give the agent a name
    ...
    # thread = project_client.agents.create_thread() # Old SDK method
    thread = project_client.agents.threads.create()
    print(f"Created thread, ID: {thread.id}")

    # Create the initial user message in the thread
    # message = project_client.agents.create_message( # Old SDK method
    ...
    # run = project_client.agents.create_and_process_run(thread_id=thread.id, agent_id=agent.id) # Old SDK method
    ...
    # run_steps = project_client.agents.list_run_steps(thread_id=thread.id, run_id=run.id) # Old SDK method
    # print(run.items())
